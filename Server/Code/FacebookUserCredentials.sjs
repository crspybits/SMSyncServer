// User credentials for both SharingUser's and OwningUser's. (Not persistently stored).
// Generic-- there are also specific classes for dealing with specific account types.
...
function UserCredentials(credentialsData) {    
    ...
    // Optional
    this.username = credentialsData[ServerConstants.accountUserName];
    
    this.userType = credentialsData[ServerConstants.userType];
    if (!isDefined(this.userType)) {
        throw new Error("No userType in credentials data!");
    }

    if (this.userType != ServerConstants.userTypeOwning && this.userType != ServerConstants.userTypeSharing) {
        throw new Error("Bad credentials data: " + this.userType);
    }

    this.accountType = credentialsData[ServerConstants.accountType];
    if (!isDefined(this.accountType)) {
        throw new Error("No accountType in credentials data!");
    }
    
    switch (this.accountType) {
    case ServerConstants.accountTypeGoogle:
        break;
    
    case ServerConstants.accountTypeFacebook:
        break;
    }
